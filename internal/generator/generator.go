...
func RenderZigRuntime(api *model.API, cfg Config) []byte {
	...
	if apiUsesStreams(api) {
		...
		body.WriteString("    const file = streamFileFromEncoded(reader);\n")
		body.WriteString("    var threaded: std.Io.Threaded = .init_single_threaded;\n")
		body.WriteString("    const io = threaded.io();\n")
		body.WriteString("    const n = file.readStreaming(io, &.{buffer}) catch |err| switch (err) {\n")
		body.WriteString("        error.EndOfStream => return error.EndOfStream,\n")
		body.WriteString("        else => return error.StreamReadFailed,\n")
		body.WriteString("    };\n")
		body.WriteString("    return n;\n")
		body.WriteString("}\n\n")
		...
		body.WriteString("    const file = streamFileFromEncoded(writer);\n")
		body.WriteString("    var threaded: std.Io.Threaded = .init_single_threaded;\n")
		body.WriteString("    const io = threaded.io();\n")
		body.WriteString("    return file.writeStreaming(io, &.{}, &.{buffer}, 1) catch { return error.StreamWriteFailed; };\n")
		body.WriteString("}\n\n")
	}
