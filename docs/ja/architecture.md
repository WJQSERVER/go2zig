...
- **特殊型**: `String`、`Bytes`、実験的な `GoReader` / `GoWriter`
...
- `WithHeaderOutput(path)`: Zig ヘッダ出力先を設定する
- `WithRuntimeZig(path)` / `WithBridgeZig(path)`: 生成ファイル位置を変更する
- `WithDynamicBuild(enabled)`: 動的 / 静的ライブラリを切り替える
- `WithTopLevelFunctions(enabled)`: トップレベル転送関数の生成を制御する
- `WithStreamExperimental(enabled)`: 実験的ストリーム橋接を有効化する
- `WithAPIModuleName(name)` / `WithImplModule(name)`: Zig `@import` モジュール名を上書きする
- `Build()`: 生成とビルドを実行する
...
補足:
- 現在サポートしている 5 ターゲットはすべてメイン CI でテスト、ベンチマーク、ビルド確認を行っています
- Linux job では `GO2ZIG_RUN_LINUX_RUNTIME_TESTS=1` によって低層 runtime 実行テストも有効化されています
