...
- `basic.dll` または `libbasic.so` - 動的ライブラリ
...
Linux 上の no-`cgo` runtime はまだ継続的に磨き込み中であり、現在のメイン CI は安定した生成、コンパイル、統合検証を重視しています。
...
3. **プラットフォーム未対応**: Windows/amd64 または Linux/amd64 上で実行しているか確認する
