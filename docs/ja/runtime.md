...
- Darwin: `dlopen` / `dlsym` / `dlclose` に基づく
...
- struct を要素に持つ名前付き slice alias

現在、slice alias は「POD slice」より広い範囲を扱えます。主な要素型は次のとおりです。
- 基本数値型
- 整数ベースの enum
- 固定長 array
- 名前付き POD slice alias
- 上記を再帰的に組み合わせた struct

## Optional プロトコル

現在安定してカバーされている optional 形状は主に `optional POD` です。

- `?primitive`
- `?enum`
- `?array` / `?array alias`

Go 側の公開型はデフォルトで `*T` にマッピングされ、現在のテストとサンプルも主にこれらの POD 形状を中心にしています。

ABI 層は Zig ネイティブ optional のレイアウトに直接依存せず、明示的な tagged wrapper を使います。
...
生成される `Go2ZigClient` は、動的ライブラリのロードとシンボル解決を遅延実行します。

- `client.Load()` を明示的に呼べば、通常の Go `error` として扱えます
- 生成メソッドを直接呼んだ場合も内部で自動ロードを試みます
- その自動ロードが失敗すると、現在の呼び出し経路は戻り値ではなく `panic(err)` になります

そのため、エラー制御を明確にしたい場合は先に `Load()` を呼ぶのが安全です。
