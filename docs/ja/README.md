...
本プロジェクトは、no-cgo 呼び出し実装において [purego](https://github.com/ebitengine/purego) を、FFI コード生成設計において [rust2go](https://github.com/ihciah/rust2go) を参照しています。
...
未対応:
- `darwin/amd64` - 現時点では未対応
- その他のアーキテクチャ - 現時点では未対応

## Platform Tiers

`purego` のサポート階層の考え方を参考にしています。

- **Tier 1** - 現在メイン CI でテスト、ベンチマーク、ビルド確認を継続実行しているターゲット: `windows/amd64`, `windows/arm64`, `linux/amd64`, `linux/arm64`, `darwin/arm64`

今後新しいターゲットを追加する場合は、まず best-effort で導入し、その後この主検証セットへ昇格させる想定です。
...
- 特殊型: `String`、`Bytes`
- slice alias: 名前付き slice alias。POD slice に加えて、要素が struct の slice alias も含む
...
1. **プラットフォーム制約**: Windows/Linux 上の `amd64` と `arm64`、および Darwin 上の `arm64` のみ対応
2. **型制約**: Go の `map`、channel、interface などの固有型は未対応
3. **メモリ管理**: 固定の割り当て方式であり、allocator はカスタマイズできない
4. **性能コスト**: 各呼び出しでデータコピーが必要
5. **ランタイムロード**: 明示的な `Load()` を省略して最初の遅延ロードに失敗した場合、現在の生成呼び出し経路は panic する
